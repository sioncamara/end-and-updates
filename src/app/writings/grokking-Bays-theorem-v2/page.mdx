...
Understanding Bayes' theorem in an intutive matter consists of taking the abstract formula known as Baye's theorem
<Math display={true}>
{"P(H|E) = \\frac{P(E|H) \\times P(H)}{P(E)}"}
</Math>

And mapping the abstract concepts these symbols respresent into conceptual models our human perceptual systems intutivley understand.
...
Now that we have an intuitive understanding of what conditional probability is, we're going to use this conceptual building block
to start understanding the type of rational inference that Bayes' theorem formally represents.

The good news is that we've already done most of the conceptual heavy lifting.

The next useful insight is that the numerator of the conditional probability formula equals the numerator of Bayes' theorem:
...
It's probably not obvious why these are equal at first glance.
