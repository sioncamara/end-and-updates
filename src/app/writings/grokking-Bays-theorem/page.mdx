...
export const metadata = generateArticleMetadata({
  ...
  slug: 'grokking-Bays-theorem',
  ogImageType: 'math',
  customFormula: 'P(H|E) = P(E|H) × P(H) / P(E)',
  highlightConcept: 'Indirect Inference Through Probability',
})
...
Okay, so we have this formula, but how do we understand it intuitively?
...
When you click on <Math>{"P(H \\cap E)"}</Math> within the formula, you'll see the intersection highlighted in purple. This intersection occupies 11% of the total space, giving it a probability of 0.11. The area occupied by any event in our probability space is called its **probability mass**.
...
Here's what's happening: conditional probability shifts our frame of reference. Instead of asking "What are the chances of H in the entire space?" we're asking "What are the chances of H within the constrained space where E is true?"
...
<CollapsibleMathBreakdown 
  ...
  steps={[
    ...
    },
    {
      ...
      explanation: "The 100s cancel out completely! Notice what happens: we're left with just the ratio of areas within the E region, as if E were our complete space."
    },
    {
      ...
      math: (
        <div className="flex flex-col xs:flex-row items-center justify-center gap-1 xs:gap-2">
          <Math display={true} className="text-lg">{"P(H|E) = \\frac{\\text{Area of } H \\cap E \\text{ within E}}{\\text{Total area of E}}"}</Math>
          <div className="flex flex-row items-center gap-1">
            <Math className="text-lg">{"="}</Math>
            <Math className="text-lg">{"\\frac{11}{23}"}</Math>
            <Math className="text-lg">{"≈ 0.48"}</Math>
          </div>
        </div>
      ),
...
Now that we have a solid intuitive grasp of conditional probability and its renormalization concept, we're ready to understand how Bayes' theorem builds upon this foundation. The key insight to be gained is that Bayes' theorem is a clever rearrangement of what we already know—this rearrangement unlocks powerful new capabilities for reasoning with uncertainty.
...
Both formulas calculate the same thing—<Math>{"P(H|E)"}</Math>—but notice how their numerators are highlighted in purple. This isn't a coincidence. The crucial connection is that these numerators are mathematically equivalent—they evaluate to the same value even though they're different expressions.
...
<CollapsibleMathBreakdown 
  ...
  steps={[
    ...
    },
    {
      ...
      explanation: "This proves why the conditional probability formula and Bayes' theorem are mathematically equivalent—they both calculate P(H|E), with numerators that evaluate to the same value. What makes this practical is that we often don't know P(H ∩ E) directly, but we do know P(E|H) and P(H). Bayes' theorem lets us use the information we actually have to calculate what we want to know."
    }
  ]}
/>
...
<div className="my-8 px-0 sm:px-6 py-6 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
  <div className="text-center space-y-8">
    ...
    <div className="flex justify-between items-start px-0 sm:px-8">
      <div className="text-center flex-1">
        <div className=" px-2 sm:px-4 py-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-600 inline-block">
      ...
      <div className="text-center flex-1">
        <div className="px-2 sm:px-4 py-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-600 inline-block">
          <div className="flex items-center gap-2">
            <Math>{"P(E|H)"}</Math>
            <Math>{"\\times"}</Math>
            <Math>{"P(H)"}</Math>
